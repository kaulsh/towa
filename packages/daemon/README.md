...
```bash
# Foreground daemon (control HTTP on 127.0.0.1:18741 by default)
towa run --config-file ./towa.yaml

# From another terminal (no config file needed):
towa ping
towa status
...
# towa status --port 9999
# TOWA_CONTROL_PORT=9999 towa status
```
